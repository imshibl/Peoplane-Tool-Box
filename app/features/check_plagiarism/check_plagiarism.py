...
def check_plagiarism(input: CheckPlagiarismModel, nlp):
    ...
    sop_text = preprocess_text_with_spacy(input.sop, nlp)
    ...
    for index, row in references_df.iterrows():
        ...
        if reference_owner != input.email and reference_type == input.sop_type:
    ...
    if highest_similarity < 0.6:
        ...
        new_data = ({"SOP": input.sop, "Owner": input.email, "Type": input.sop_type},)
